...
watch(
    () => props.show,
    async (show) => {
        if (!props.canUseFocusTrap) return;
        await nextTick();
        if (show) activate();
        else deactivate();
    },
        ...
        <Teleport to="body">
            <Transition @enter="onEnter" @leave="onLeave">
                <div
                    v-if="show"
                    ref="float"
                    class="absolute z-[1000] overflow-hidden rounded font-semibold shadow-xl dark:bg-dark-800"
                >
